				...
				void *private_data)
{
	...
	case AUDDEV_EVT_DEV_RLS: {
        ...
        case AUDDEV_EVT_VOICE_STATE_CHG: {
                ...
                audio->call_state = evt_payload->voice_state;
                if (audio->in_call && audio->running) {
                        if (audio->call_state == VOICE_STATE_INCALL)
                                audpcm_in_record_config(audio, 1);
                        else if (audio->call_state == VOICE_STATE_OFFCALL) {
                                audpcm_in_record_config(audio, 0);
                                wake_up(&audio->wait);
                        }
                }
	...
	case AUDREC_CMD_MEM_CFG_DONE_MSG: {
		...
		audio->running = 1;
                ...
                if ((!audio->in_call && (audio->dev_cnt > 0)) ||
                        (audio->in_call &&
                                (audio->call_state == VOICE_STATE_INCALL)))
#else
		if (audio->dev_cnt > 0)
#endif
			audpcm_in_record_config(audio, 1);
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_START: {
		...
		MM_DBG("AUDIO_START\n");
#ifdef CONFIG_UNLOCK_184MHZ
                if (audio->in_call && (audio->call_state != VOICE_STATE_INCALL)) {
                         rc = -EPERM;
                         break;
                }
#endif
		...
		rc = wait_event_interruptible(
			...
			audio->abort);
                        ...
                        audio->abort || (audio->in_call && audio->running &&
                                 (audio->call_state == VOICE_STATE_OFFCALL)));
#endif
...
static int audpcm_in_open(struct inode *inode, struct file *file)
{
	...
	audio->device_events = AUDDEV_EVT_DEV_RDY | AUDDEV_EVT_DEV_RLS |
        ...
        audio->call_state = msm_get_call_state();
#endif
