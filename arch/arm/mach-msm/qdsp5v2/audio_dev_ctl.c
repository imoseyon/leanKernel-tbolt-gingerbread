...
void broadcast_event(u32 evt_id, u32 dev_id, u32 session_id)
{
	...
	for (; ;) {
		...
		// imosey - may want to remove this block
		if (evt_id == AUDDEV_EVT_START_VOICE)
		...
		if (callback->clnt_type == AUDDEV_CLNT_AUDIOCAL) {
			...
			if (evt_id == AUDDEV_EVT_VOICE_STATE_CHG) {
			   ...
			   evt_payload->audcal_info.dev_type =
				(dev_info->capability & SNDDEV_CAP_TX) ? SNDDEV_CAP_TX : SNDDEV_CAP_RX;
			   evt_payload->audcal_info.sample_rate = dev_info->set_sample_rate ?
				dev_info->set_sample_rate : dev_info->sample_rate;
                        }
                        if (evt_payload->audcal_info.dev_type == SNDDEV_CAP_TX) {
                                if (session_id == SESSION_IGNORE)
                                        temp_sessions = dev_info->sessions;
                                else {
                                    temp_sessions = session_id;
                                    evt_payload->audcal_info.sessions =
                                        (temp_sessions >> ((AUDDEV_CLNT_ENC-1) * 8));
				}
                        } else {
                                if (session_id == SESSION_IGNORE)
                                        temp_sessions = dev_info->sessions;
                                    ...
                                    temp_sessions = session_id;
                                    evt_payload->audcal_info.sessions =
                                         (temp_sessions >> ((AUDDEV_CLNT_DEC-1) * 8));
				}
                        }
			callback->auddev_evt_listener(evt_id, evt_payload, callback->private_data);
