...
static struct android_usb_function usbnet_function = {
	.name = USBNET_FUNCTION_NAME,
	.bind_config = usbnet_bind_config,
};

static struct platform_driver usbnet_driver = {
	.driver = { .name = USBNET_FUNCTION_NAME, },
};

static struct platform_device usbnet_platform_device = {
	.name = USBNET_FUNCTION_NAME,
};
static int __init init(void)
{
	...
	r = platform_driver_register(&usbnet_driver);
	if (r < 0)
		return r;
	r = platform_device_register(&usbnet_platform_device);
	if (r < 0) {
		platform_driver_unregister(&usbnet_driver);
		return r;
	}
